...
export default function ProfilePage() {
  ...
  // ─── Load profile: merge Django data + localStorage extras ───────────────
  useEffect(() => {
    if (!user) return;

    const loadProfile = async () => {
      // 1. Local extras (department, level, dob, gender, campus, avatar)
      ...
      try {
        ...
        if (res.ok) {
          ...
          setProfile({
            name: djangoUser.username || parsed.name || "",
            email: djangoUser.email || "",
            phone: djangoUser.phone || parsed.phone || "",
            bio: djangoUser.bio || parsed.bio || "",
            whatsapp: djangoUser.whatsapp || parsed.whatsapp || "",
        ...
        console.warn("Failed to load Django profile, using local data", e);
      }

      // Fallback to local + auth store
      setProfile({
        name: user.username || parsed.name || "",
        email: user.email || "",
  ...
  const completedCount = completionFields.filter(
    f => !!profile[f.key as keyof Profile]
  ).length;
  ...
  // ─── Save: sync to Django + localStorage ─────────────────────────────────
  const saveProfile = async () => {
    if (!storageKey) return;
    setSaving(true);
    setSaveError("");

    // 1. Save non-Django extras locally (department, level, dob, gender, campus, avatar)
    const localExtras = {
    ...
    // 2. PATCH Django with fields it owns
    let djangoSyncOk = false;
    try {
      const res = await fetchWithAuth(`${API_URL}/api/auth/profile/update/`, {
        ...
        body: JSON.stringify({
          ...
          hostel: profile.campus, // map campus → hostel if applicable
        }),
      });
      djangoSyncOk = res.ok;
      if (!res.ok) {
      ...
      setSaveError("Network error — local changes saved, but server sync failed.");
    }

    // 3. Check profile completion on Django
    const nowComplete = completionFields.every(
      f => !!profile[f.key as keyof Profile]
    );
    if (nowComplete && !bonusGranted) {
      try {
        const res = await fetchWithAuth(
          `${API_URL}/api/auth/profile/check-completion/`,
          { method: "POST" }
        );
        if (res.ok) {
          const data = await res.json();
      ...
      <div className="p-5 pb-32 space-y-6 bg-gray-50 dark:bg-gray-950 min-h-screen">
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-green-700 dark:text-green-400">
                5% discount applied to your first order
              </p>
            </div>
          </motion.div>
        )}
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-5 shadow-sm border border-gray-200 dark:border-gray-800">
          ...
          <div className="space-y-5">
            {fields.map(({ label, name, icon: Icon, type, options, editable, note, multiline }) => (
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { icon: Store,       label: "Sold",     value: stats.sold,
              from: "from-purple-50", to: "to-indigo-50",
              iconColor: "text-purple-600", numColor: "text-purple-700" },
            { icon: ShoppingBag, label: "Bought",   value: stats.bought,
              from: "from-teal-50",   to: "to-emerald-50",
              iconColor: "text-teal-600",   numColor: "text-teal-700" },
            { icon: Heart,       label: "Wishlist", value: stats.wishlist,
              from: "from-pink-50",   to: "to-purple-50",
              iconColor: "text-pink-600",   numColor: "text-pink-700" },
