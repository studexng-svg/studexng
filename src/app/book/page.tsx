...
export default function MyBookingsPage() {
  ...
  const handlePayNow = (b: ApiBooking) => {
    setBooking({
      ...
      providerImg: b.listing.images?.[0]?.image || "", // empty string — no placeholder
      ...
      <div className="px-6 py-8 space-y-10 pb-32">
        ...
        {!loading && error && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-5 text-center">
            ...
            <button onClick={fetchBookings} className="mt-3 text-sm text-red-600 underline font-semibold">Try again</button>
          </div>
        )}
              ...
              <div className="bg-gradient-to-br from-purple-50 to-teal-50 rounded-3xl p-8 text-center border border-purple-200">
                ...
                <Link href="/categories">
                  <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }}
                    ...
                    Browse Categories <ChevronRight className="w-6 h-6" />
                  </motion.button>
                </Link>
              </div>
            ) : (
                  ...
                  return (
                    <motion.div key={b.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                      ...
                      <div className="flex gap-4 mb-4">
                        <div className="w-16 h-16 rounded-2xl overflow-hidden bg-purple-100 flex-shrink-0 flex items-center justify-center">
                          {b.listing.images?.[0]?.image ? (
                            <Image src={b.listing.images[0].image} alt={b.listing.title}
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-black text-lg leading-tight truncate">{b.listing.title}</h3>
                          <p className="text-sm text-gray-500 mt-0.5">by {b.listing.vendor.username}</p>
                          <div className="flex flex-wrap gap-3 mt-2 text-sm text-gray-600">
                            <span className="flex items-center gap-1"><CalendarCheck className="w-4 h-4" />{b.scheduled_date}</span>
                            <span className="flex items-center gap-1"><Clock className="w-4 h-4" />{b.scheduled_time}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between items-center">
                        <p className="font-black text-2xl text-purple-700">₦{b.listing.price.toLocaleString()}</p>
                        <div className="flex gap-2">
                          {b.status === "confirmed" && (
                            <motion.button whileTap={{ scale: 0.95 }} onClick={() => handlePayNow(b)}
                          ...
                          {["pending", "confirmed"].includes(b.status) && (
                            <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleCancel(b.id)}
          ...
          <section>
            ...
            <div className="space-y-4">
              {past.map((b) => {
                ...
                const StatusIcon = cfg.icon;
                return (
                  <div key={b.id} className="bg-gray-50 rounded-3xl p-5 border border-gray-100">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-500">{b.scheduled_date} at {b.scheduled_time}</p>
                        <div className={`inline-flex items-center gap-1 mt-2 px-2 py-1 rounded-lg text-xs font-bold border ${cfg.color}`}>
                          <StatusIcon className="w-3 h-3" />{cfg.label}
                        </div>
                      </div>
                      <p className="font-black text-lg text-gray-700 self-center">₦{b.listing.price.toLocaleString()}</p>
                    </div>
                  </div>
                );
              })}
