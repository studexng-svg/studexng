...
// Uses a plain <img> tag — no Next.js domain restrictions, works with any
// backend URL (local Django, Render, Cloudinary). Falls back to sparkle emoji.
function ListingImage({
  ...
  // Build absolute URL — Django may return a relative path like /media/listings/file.jpg
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `${API_URL}/api/services/listings/?search=${encodeURIComponent(searchQuery)}`
        );
      ...
      >
        <div className="flex items-center justify-between p-4 gap-3">
          <Link href="/home" className="flex items-center gap-2 flex-shrink-0">
            ...
            />
            <span className="font-black text-xl bg-gradient-to-r from-teal-600 to-purple-600 bg-clip-text text-transparent">
              StudEx
            </span>
          </Link>
            ...
            {isLoggedIn ? (
                    ...
                    >
                      {searching ? (
                        ...
                        searchResults.map(item => (
                          <Link
                            key={item.id}
                            href={`/category/${item.category?.slug || item.category}`}
                            ...
                            <div className="flex items-center gap-3 p-3 hover:bg-purple-50 dark:hover:bg-gray-700 transition border-b border-gray-50 dark:border-gray-700 last:border-0 cursor-pointer">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-500">
                                  by @{item.vendor?.username || item.vendor}
                                </p>
                              </div>
              ...
              <div className="flex gap-2">
                <Link href="/auth">
                  <motion.button
                    {...buttonHover}
                    className="flex items-center gap-1 px-4 py-2 bg-gradient-to-r from-teal-500 to-purple-600 text-white rounded-full text-sm font-bold shadow"
                  >
                ...
                <Link href="/auth">
                  <motion.button
                    {...buttonHover}
                    className="flex items-center gap-1 px-4 py-2 bg-white dark:bg-gray-800 text-purple-600 rounded-full text-sm font-bold border-2 border-purple-200 dark:border-gray-700"
                  >
        ...
        <div className="p-4 space-y-6 pb-28">
            ...
            <Link href="/categories">
              ...
              >
                ...
                <div className="relative z-10 h-full flex flex-col justify-end">
                  ...
                  <p className="text-base mt-1 text-white/90 font-medium">
                ...
                categories.map((cat, i) => (
                  ...
                  >
                    <Link href={`/category/${cat.slug}`}>
                      <motion.div
                        {...cardHover}
                        className="bg-white dark:bg-gray-800 p-3 rounded-xl text-center shadow border border-gray-100 dark:border-gray-700 cursor-pointer"
                      >
                        {/* Category image — plain img, no domain restrictions */}
                        <div className="relative w-full h-24 rounded-lg overflow-hidden mb-2 bg-gradient-to-br from-purple-100 to-teal-100">
                          <ListingImage src={cat.image} alt={cat.title} />
                        </div>
          ...
          <div className="flex justify-center">
            <Link href="/categories">
              <motion.button
                whileHover={{ x: 4 }}
                className="flex items-center gap-2 text-gray-600 dark:text-gray-400 font-medium text-sm hover:underline"
              >
          ...
          <motion.div {...fadeInUp}>
            <div className="flex justify-between items-center mb-3">
              ...
              <Link href="/categories" className="text-purple-600 dark:text-purple-400 text-sm font-medium">
                See All
              </Link>
            </div>
            <div className="flex space-x-3 overflow-x-auto pb-2" style={{ scrollbarWidth: "none" }}>
                ...
                listings.slice(0, 6).map((listing, i) => (
                  ...
                  >
                    <Link href={`/listing/${listing.id}`}>
                      <motion.div
                        {...cardHover}
                        className="bg-white dark:bg-gray-800 p-3 rounded-xl min-w-[155px] shadow border border-gray-100 dark:border-gray-700 relative cursor-pointer"
                      >
                        {/* Listing image — plain img tag, shows actual photos */}
                        ...
                        >
                          <Heart
                            className={`w-3.5 h-3.5 ${
                              mounted && isInWishlist(listing.id)
                        ...
                        <p className="text-xs text-gray-500 mb-1">
                          by @{listing.vendor?.username || listing.vendor}
                        </p>
