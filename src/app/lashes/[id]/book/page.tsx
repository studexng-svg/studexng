// src/app/lashes/[id]/book/page.tsx  ← FINAL STUDEx 2.0 BOOKING PAGE (WORKS 100%)
"use client";
...
import { useBookingStore } from "@/lib/bookingStore"; // ← NEW STORE
...
export default function BookingPage() {
  ...
  const travelFee = location.includes("Event Venue") ? 2000 : 0;
      ...
      <div className="px-6 pt-8 pb-40 space-y-10">
        ...
        <section>
          ...
          <div className="space-y-3">
            {["Provider's Studio", "My Hostel", "Event Venue (+₦2,000)"].map((loc) => (
              <label
                key={loc}
              ...
              >
                ...
                {loc.includes("(+") && <span className="text-purple-600 font-black">+₦2,000</span>}
              </label>
            ))}
          </div>
        </section>
        ...
        {/* NOTE */}
        <section>
          <h3 className="text-2xl font-black mb-4">Note to Provider (Optional)</h3>
          <textarea
            value={note}
            onChange={(e) => setNote(e.target.value)}
            placeholder="e.g. I want wispy volume, medium length..."
        ...
        <div className="bg-gradient-to-r from-purple-600 to-teal-600 text-white rounded-3xl p-8 shadow-2xl">
          ...
          <div className="space-y-3 text-lg opacity-90">
            <div className="flex justify-between"><span>Base Service</span><span>₦{basePrice.toLocaleString()}</span></div>
            {addonPrice > 0 && <div className="flex justify-between"><span>Add-ons</span><span>₦{addonPrice.toLocaleString()}</span></div>}
            {travelFee > 0 && <div className="flex justify-between"><span>Travel Fee</span><span>+₦2,000</span></div>}
          </div>
        </div>
        ...
        <div className="bg-purple-50 rounded-3xl p-6 text-center border-2 border-purple-200">
          ...
          <p className="text-gray-700 mt-2">You pay now. {p.name.split(" ")[0]} gets paid only after you say "Perfect"</p>
        </div>

        {/* PAY BUTTON — NOW SAVES BOOKING */}
        <motion.button
          onClick={handlePay}
          whileTap={{ scale: 0.95 }}
