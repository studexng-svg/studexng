...
import { User, Phone, MapPin, Lock, Eye, EyeOff, Mail, ArrowLeft, CheckCircle2, XCircle, Loader2 } from "lucide-react";
...
const validateEmail = (v: string) => {
  if (!v) return { ok: false, msg: "" };
  if (!v.includes("@")) return { ok: false, msg: "Enter your school email" };
...
const validatePassword = (v: string) => {
  ...
  return {
    ok: allOk,
    msg: allOk ? "Strong password ✓" : "Password doesn't meet requirements",
    checks,
  };
};
...
function PasswordStrength({ checks }: { checks: { length: boolean; upper: boolean; lower: boolean; number: boolean } }) {
  ...
  return (
    <div className="mt-2 space-y-2">
      ...
      {passed > 0 && (
        <p className={`text-xs font-bold ${colors[passed - 1].replace("bg-", "text-")}`}>
          {labels[passed - 1]}
        </p>
      )}
      {/* Checklist */}
...
export default function UserSignup() {
  ...
  // Touched state — only show errors after user has interacted with field
  ...
  useEffect(() => {
    ...
    if (!v.ok || !form.username) { setUsernameAvailable(null); return; }
  ...
  const generateOTP = () => {
    ...
    setSentOtp(code);
    // In production this would be sent via SMS — for now alert
    alert(`[DEV] Your OTP: ${code}`);
  ...
  const handleStep1 = (e: React.FormEvent) => {
    ...
    if (!step1Valid) { setMessage("Please fix the errors above before continuing."); return; }
    setMessage("");
    generateOTP();
    setStep(2);
  };
  ...
  const handleStep2 = async (e: React.FormEvent) => {
    e.preventDefault();
    if (otp !== sentOtp) { setMessage("Wrong OTP. Try again."); return; }
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">
            {step === 1 ? "Join the PAU movement in 30 seconds" : `Enter the 6-digit code sent to ${form.phone}`}
          </p>
        </div>
        ...
        <div className="flex justify-center gap-2 mb-8">
          {[1, 2].map(i => (
            <div key={i} className={`h-2 rounded-full transition-all duration-300 ${i === step ? "w-8 bg-purple-600" : i < step ? "w-4 bg-teal-500" : "w-4 bg-gray-200"}`} />
          ))}
        </div>
            ...
            {step === 1 ? (
              <motion.div key="step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-5">
                ...
                <div>
                  ...
                  {/* Format validation */}
                  <FieldFeedback show={touched.username && !!form.username} ok={usernameVal.ok} msg={usernameVal.msg} />
                  {/* Availability */}
                  {touched.username && form.username && usernameVal.ok && !usernameChecking && (
                    <FieldFeedback
                      show={usernameAvailable !== null}
                      ok={usernameAvailable === true}
                      msg={usernameAvailable === true ? "Username is available!" : "Username already taken"}
                    />
                  )}
                  {!touched.username && (
                ...
                <div>
                  ...
                  <input
                    ...
                    autoComplete="email"
                  />
                  <FieldFeedback show={touched.email && !!form.email} ok={emailVal.ok} msg={emailVal.msg} />
                  {!touched.email && (
                ...
                <div>
                  ...
                  />
                  <FieldFeedback show={touched.phone && !!form.phone} ok={phoneVal.ok} msg={phoneVal.msg} />
                  {!touched.phone && (
                ...
                <div>
                  ...
                  <FieldFeedback show={touched.hostel && !hostelOk} ok={false} msg="Please select your hostel" />
                </div>
          ...
          <AnimatePresence>
            {message && (
              <motion.p
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0 }}
                className={`text-center font-bold text-sm px-4 py-3 rounded-xl ${
                  message.includes("🎉") || message.includes("sent") || message.includes("good") || message.includes("OTP")
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          ...
          {/* ✅ Fixed: use Link instead of router.push to remove red line */}
          <a
            href="/auth"
            className="font-black text-purple-600 underline hover:text-purple-800 cursor-pointer"
          >
            Login here
          </a>
        </p>
