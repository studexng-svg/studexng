...
/**
 * Shared by both src/app/vendor/[username]/page.tsx and
 * src/app/store/[username]/page.tsx — Store vs Vendor is a routing/display
 * distinction only, not a different data model (see accounts/models.py
 * Vendor/VendorType). Each route's own page.tsx is a thin server component
 * that decides whether to redirect (wrong route for this vendor's type) or
 * render this exact component — so there is exactly one implementation of
 * the profile page, not two.
 */
export default function VendorProfileClient() {
  ...
  const handleShare = async () => {
    // Share whatever URL the browser is actually on — by the time this client
    // component renders, each route's own page.tsx has already redirected
    // away if this vendor's type doesn't match /vendor/ vs /store/, so
    // location.pathname is always the correct, current one.
  ...
  // Single source of truth: the same is_menu_vendor field VendorListSerializer
  // exposes everywhere else (accounts/serializers.py), not a listings-based
  // guess. A Store vendor with zero listings yet still renders as a Store.
  const isMenuVendor = !!vendor.is_menu_vendor;
  const catalogLabel = vendor.catalog_label || "Menu";

  const TABS: { id: Tab; label: string; count?: number }[] = [
    { id: "listings", label: isMenuVendor ? catalogLabel : "Listings", count: sorted.length },
