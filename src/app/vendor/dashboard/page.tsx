          ...
          {filtered.map(booking => (
            <div key={booking.id} className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                {/* Price — what you receive after the ₦200 service fee */}
                <div className="bg-gray-800 rounded-xl p-3 col-span-2">
                  ...
                  <p className="text-gray-600 text-xs mt-0.5">
                    (listing price ₦{Number(booking.listing_price || 0).toLocaleString()} minus ₦200 service fee)
                  </p>
                </div>
              </div>
...
function EarningsTab() {
  ...
  // Three stat cards — total earned, total orders, service fee note
  const stats = [
    {
      label: "Total Earned",
    ...
    },
    {
      ...
      value: "₦200",
      color: "text-gray-400",
      bg: "from-gray-800/50 to-gray-900/10",
  ...
  return (
    <div className="overflow-y-auto space-y-6 pb-4" style={{ maxHeight: "calc(100vh - 200px)" }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map(stat => (
          <div key={stat.label} className={`bg-gradient-to-br ${stat.bg} border border-gray-800 rounded-2xl p-5`}>
            <p className="text-gray-500 text-xs mb-2">{stat.label}</p>
            <p className={`text-2xl font-black ${stat.color}`}>{stat.value}</p>
            <p className="text-gray-600 text-xs mt-1">{stat.note}</p>
          </div>
        ))}
      </div>

      {/* How payouts work — replaces the old commission tiers block */}
      <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
        ...
        <div className="space-y-3 text-sm text-gray-400 leading-relaxed">
          <div className="flex items-start gap-3 bg-gray-800 rounded-xl p-3">
            <span className="text-teal-400 font-black text-lg leading-none">1</span>
            <p>A customer pays the full listing price through Flutterwave checkout.</p>
          </div>
          <div className="flex items-start gap-3 bg-gray-800 rounded-xl p-3">
            <span className="text-teal-400 font-black text-lg leading-none">2</span>
            <p>Flutterwave instantly splits the payment — <span className="text-white font-bold">₦200 goes to StudEx</span> as a flat service fee.</p>
          </div>
          <div className="flex items-start gap-3 bg-gray-800 rounded-xl p-3">
            <span className="text-teal-400 font-black text-lg leading-none">3</span>
            <p><span className="text-teal-400 font-bold">Everything else goes directly to your bank account</span> — no holding period on our end. Flutterwave's own settlement cycle applies (typically T+1 or T+2 business days).</p>
          </div>
        </div>
      </div>
          ...
          <div className="divide-y divide-gray-800">
            {transactions.map((tx: any) => (
              <div key={tx.id} className="p-4 flex items-center justify-between">
                <div>
                  <p className="font-semibold text-white text-sm">{tx.service_name || `Order #${tx.order_id}`}</p>
                  <p className="text-xs text-gray-500">
                    {tx.buyer_name} · {new Date(tx.created_at).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}
                  </p>
                </div>
                <div className="text-right">
                  <p className="font-bold text-teal-400">₦{Number(tx.seller_amount).toLocaleString()}</p>
        ...
        <div className="bg-gray-900 rounded-2xl p-6 mb-6 border border-gray-700">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={resetForm} className="px-6 py-3 bg-gray-800 hover:bg-gray-700 rounded-xl font-bold text-gray-400 text-sm transition">
                Cancel
              </button>
            </div>
          </div>
            ...
            <div key={order.id} className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="text-right">
                  ...
                  <p className="font-bold text-white">₦{Number(order.amount).toLocaleString()}</p>
                </div>
                <div className="text-right">
