  ...
  return (
    <>
      ...
      {/*
        ...
        Mobile (default):
          - bottom-[70px] lifts the panel above the bottom nav (nav is ~70px tall)
          - h-[calc(100vh-70px)] fills the remaining screen above the nav
          This means the input bar at the bottom of the panel is always
          visible above the nav, never hidden behind it.

        Desktop (md+):
          - Reverts to the original right-side floating panel positioning
      */}
      <div className="
      ...
      ">
              ...
              <div key={msg.id}
                ...
                {msg.isSystem ? (
                  ...
                  <div className={`rounded-2xl px-4 py-2.5 ${
                    msg.is_mine ? 'bg-purple-600 text-white rounded-br-sm' : 'bg-white/10 text-white rounded-bl-sm'
                  }`}>
                    {!msg.is_mine && (
        ...
        <div className="bg-slate-900/90 border-t border-white/10 p-4 flex gap-2 flex-shrink-0">
          ...
          <button onClick={() => fileInputRef.current?.click()}
            className="p-3 bg-white/10 hover:bg-white/20 rounded-xl transition flex-shrink-0" title="Attach image">
