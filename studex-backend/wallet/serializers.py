# wallet/serializers.py (add this at the top of views.py file or in a separate file)
...
@permission_classes([IsAuthenticated])
def get_or_create_bank_account(request):
...
def get_banks(request):
    ...
    def release_escrow(self, request):
        ...
        try:
            ...
            seller_wallet.balance += escrow.seller_amount
            seller_wallet.save()
            ...
            if admin:
                ...
                admin_wallet.balance += escrow.platform_fee
                admin_wallet.save()
...
urlpatterns = [
    ...
    path('api/wallet/bank-account/', BankAccountViewSet.as_view({
        'get': 'detail',
        'post': 'detail',
        'put': 'detail'
    }), name='bank-account'),
    path('api/banks/', get_banks, name='get-banks'),
]